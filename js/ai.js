...
(function () {
  ...
  // Boss 弹幕/激光伤害缩放:与普通敌 projDmg 完全对齐(diff.dmgMul × dmgFactor(t) × endlessMul)
  function dmgScale(st) {
    ...
    return SV.Config.CURVES.dmgFactor(t) * diff.dmgMul * em;
  }
  ...
  const Beh = {
    ...
    boss: function (e, p, dt) {
      ...
      if (e.bossType === "duke") {
        ...
        if (e.t2 <= 0) { e.t2 = 3.5; spiralBurst(st, e, 12, 140, 10, 7); }
        ...
        if (e.t1 <= 0) { e.t1 = rate; aimedSpread(st, e, p, 3, 0.32, 260, 10); }
      } else if (e.bossType === "architect") {
        toPlayer(e, p, e.speed);
        e.t1 -= dt; e.t2 -= dt;
        if (e.t1 <= 0) { e.t1 = 2.2; spiralBurst(st, e, 10, 160, 12, 7); }
        if (e.t2 <= 0) {
        ...
        e.t3 = (e.t3 || 0) - dt;
        if (e.t3 <= 0) { e.t3 = 4.5; ringFrom(st, e.x - 50, e.y, 6, 150, 10, e.color, 6, "architect"); ringFrom(st, e.x + 50, e.y, 6, 150, 10, e.color, 6, "architect"); }
        ...
        if (e.t2 <= 0) { e.t2 = 3; spiralBurst(st, e, 14, 150, 9, 6); }
        ...
        if (e.t1 <= 0) {
          ...
          ringFrom(st, ox, oy, 10, 150, 11, e.color, 6, "inquisitor");
          spiralBurst(st, e, 12, 160, 11, 7);
        }
        if (e.t2 <= 0) { e.t2 = 1.4; aimedSpread(st, e, p, 3, 0.3, 280, 10); }
        ...
        e.t1 -= dt; e.t2 -= dt;
        if (e.t1 <= 0) { e.t1 = 6; e.cstate = "pull"; e.ct = 1.2; ringFrom(st, e.x, e.y, 12, 150, 10, e.color, 6, "magnetwarper"); }
        if (e.cstate === "pull") {
          e.ct -= dt;
          e.vx = 0; e.vy = 0;
          if (e.ct > 0) {
        ...
        if (e.t2 <= 0) {
          e.t2 = 0.9;
          if (U.dist(e.x, e.y, p.x, p.y) < 115) E.damagePlayer(st, 8 * dmgScale(st), false, "magnetwarper");
        }
          ...
          if (e.ct <= 0) {
            ...
            for (let i = 0; i < st.enemies.length; i++) {
              ...
              if (o !== e && o.bossType === "twins" && o.hp > 0) {
                ...
                ringFrom(st, e.x, e.y, 8, 150, 9, e.color, 6, "twins"); // 换位后两点各开一环
                ringFrom(st, o.x, o.y, 8, 150, 9, o.color, 6, "twins");
                break;
              }
            }
          }
        }
        if (e.t2 <= 0) { e.t2 = 2.2; aimedSpread(st, e, p, 3, 0.3, 240, 8); }
      } else if (e.bossType === "colossus") {
        // 弹幕巨像:不动 + 周期旋转扫射激光(期间召唤小怪)
        e.vx = 0; e.vy = 0;
        e.t1 -= dt;
        if (e.t1 <= 0) { e.t1 = 9; e.cstate = "sweep"; e.ct = 6; e.cdir = U.rand(0, U.TAU); spiralBurst(st, e, 14, 150, 12, 7); }
        if (e.cstate === "sweep") {
          e.ct -= dt;
          e.cdir += 1.5 * dt;
          ...
          if (proj > 0 && proj < 600 && perp < 16 + p.r) E.damagePlayer(st, 10 * dmgScale(st), false, "colossus");
          ...
          e.t2 -= dt;
          if (e.t2 <= 0) { e.t2 = 3; spiralBurst(st, e, 10, 150, 12, 8); }
        }
      } else {
        toPlayer(e, p, e.speed);
      }
