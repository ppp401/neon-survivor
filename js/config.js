...
(function () {
  ...
  const EVOLUTIONS = {
    blade: { to: "blade_evo", name: "死亡之轮", reqPassive: "maxhp", desc: "8 刃环绕，半径 +50%，持续切割近身敌人。", color: "#ffd0a0", icon: "☀" },
  ...
  const CHARACTERS = {
    ...
    overclocker: { name: "超频者", title: "过载核心", icon: "⚡", color: "#ffb25a", startWeapons: ["missile", "chain", "shotgun", "grenade", "railgun", "meteor"], startPassives: { cooldown: 1 }, hpMul: 0.9, speedMul: 1.0, special: "overclocker", mechanics: { interval: 8, duration: 2.5, damageMul: 1.2, frequencyMul: 1.2, incomingMul: 1.25 }, ability: { trigger: "每 8s 过载 2.5s", base: "武器伤害 ×1.2、攻击频率 ×1.2、承伤 ×1.25", links: "覆盖冷却、光环、炮塔、连续命中与融合间隔；不缩短 DoT/控制" }, desc: "HP ×0.9；周期进入高输出、高风险过载窗口。", appearance: { shape: "diamond", deco: "spark" } },
  ...
  const BOSSES = {
    duke: { name: "肥胖公爵", icon: "☠", hp: 980, speed: 40, dmg: 25, attacks: { projectile: [10] }, mechanics: { summonInterval: 5, summonCount: 3, ringInterval: 3.2, ringShots: 12, ringSpeed: 165 }, r: 40, color: "#d65a8a", xp: 60, tier: 1, shape: "hex", pattern: "crown", shotStyle: "ring", skill: "召唤僵尸 + 环形弹幕" },
    wraith: { name: "双生怨灵", icon: "☾", hp: 800, speed: 100, dmg: 18, attacks: { projectile: [13] }, mechanics: { orbitRadius: 250, orbitRate: 1.3, attackInterval: 1.8, shotSpeed: 260, enragedOrbitMul: 1.6, enragedInterval: 0.9, enrageRingInterval: 4.2, enrageRingShots: 8, enrageRingSpeed: 190 }, r: 24, color: "#9b6bff", xp: 50, tier: 2, count: 2, shape: "diamond", pattern: "crescent", shotStyle: "bolt", skill: "环绕飞行 + 扇形弹幕(同伴死则狂暴加速并释放弹环)" },
    scavenger: { name: "拾荒机兵", icon: "⚙", hp: 875, speed: 62, dmg: 23, attacks: { projectile: [10] }, mechanics: { warn: 0.9, interval: 4.5, chargeSpeed: 270, chargeDuration: 0.45, recovery: 0.75, exitShots: 5, exitSpread: 0.24, shotSpeed: 220 }, r: 35, color: "#f1ad62", xp: 55, tier: 1, shape: "square", pattern: "nodes", shotStyle: "bolt", skill: "追击 + 预警冲刺 + 冲刺终点扇射" },
    frostwarden: { name: "霜壳守卫", icon: "❄", hp: 1035, speed: 44, dmg: 22, attacks: { projectile: [9] }, mechanics: { warn: 0.7, follow: 0.5, interval: 3.4, flankAngle: 0.34, flankSpeed: 180, centerSpeed: 195 }, r: 39, color: "#8ddfff", xp: 65, tier: 1, shape: "hex", pattern: "double_hex", shotStyle: "rune", skill: "锁定方位，先射两侧冰弹，再补中路冰弹" },
    bloodhunter: { name: "血棘猎手", icon: "✦", hp: 900, speed: 62, dmg: 20, attacks: { projectile: [9] }, mechanics: { range: 245, swayRate: 3, swayAngle: 0.38, warn: 0.75, interval: 3.6, flankDist: 58, spread: 0.16, shotSpeed: 215 }, r: 32, color: "#ff5278", xp: 60, tier: 1, shape: "triangle", pattern: "bloodthorn", shotStyle: "bolt", skill: "锁定玩家位置，从两侧发射交汇棘弹" },
    riftsentry: { name: "裂隙哨兵", icon: "◇", hp: 990, speed: 46, dmg: 22, attacks: { projectile: [8, 9] }, mechanics: { warn: 0.8, interval: 4.0, portalDist: 75, spread: 0.22, shotSpeed: 190 }, r: 38, color: "#a58bff", xp: 65, tier: 1, shape: "diamond", pattern: "rift", shotStyle: "rune", skill: "从成对裂隙向锁定位置交叉射击" },
    thornwarden: { name: "铁棘卫士", icon: "✥", hp: 1000, speed: 48, dmg: 22, attacks: { projectile: [9] }, mechanics: { warn: 0.8, recovery: 0.65, interval: 3.5, armor: 0.25, spread: 0.38, shotSpeed: 215 }, r: 37, color: "#c47a9b", xp: 65, tier: 1, shape: "hex", pattern: "shield", shotStyle: "bolt", skill: "展开护甲后发射扇形弹幕，射后短暂硬直" },
    queen: { name: "蜂后", icon: "☼", hp: 2200, speed: 30, dmg: 28, attacks: { projectile: [13] }, mechanics: { summonInterval: 5, summonCount: 4, ringInterval: 2.6, ringShots: 14, ringSpeed: 170 }, r: 46, color: "#ff6ab0", xp: 100, tier: 2, shape: "hex", pattern: "honey", shotStyle: "ring", skill: "召唤蜂群 + 环形/螺旋弹幕" },
    magnetwarper: { name: "磁暴行者", icon: "⚡", hp: 1500, speed: 45, dmg: 20, attacks: { projectile: [13], shock: [11] }, mechanics: { moveMul: 0.7, pullInterval: 5.2, pullDuration: 1.2, pullForce: 120, ringShots: 12, ringSpeed: 175, shockInterval: 0.9, shockRange: 115, releaseShots: 8, releaseSpeed: 230 }, r: 36, color: "#8e7bff", xp: 90, tier: 2, shape: "star", pattern: "poles", shotStyle: "rune", skill: "引力波把玩家吸向自身 + 释放弹环 + 贴身电击圈" },
    twins: { name: "镜像双子", icon: "◐", hp: 1050, speed: 60, dmg: 15, attacks: { projectile: [12, 13] }, mechanics: { swapInterval: 6.5, swapWarn: 0.25, ringShots: 8, ringSpeed: 180, shotInterval: 1.8, shotSpeed: 260, enragedSpeedMul: 1.5 }, r: 28, color: "#7df9ff", xp: 80, tier: 2, count: 2, shape: "triangle", pattern: "split", shotStyle: "bolt", skill: "追击 + 周期换位(杀其一,本体反噬 25% 并狂暴)" },
    stormherald: { name: "雷暴使徒", icon: "⚡", hp: 1550, speed: 52, dmg: 22, attacks: { projectile: [12] }, mechanics: { range: 260, sweep: 1.05, shotInterval: 0.14, interval: 2.4, sweepAngle: 1.35, shotSpeed: 310 }, r: 34, color: "#aaa0ff", xp: 95, tier: 2, shape: "star", pattern: "storm", shotStyle: "bolt", skill: "侧向游走 + 扫角电弹连射" },
    bloodoracle: { name: "血谕祭司", icon: "✧", hp: 1650, speed: 42, dmg: 24, attacks: { projectile: [12] }, mechanics: { ritualWarn: 0.8, ritualInterval: 6, ritualShots: 3, ritualSpread: 0.2, shotSpeed: 230, boltInterval: 2.7, boltShots: 3, boltSpread: 0.28 }, r: 38, color: "#ff718e", xp: 100, tier: 2, shape: "cross", pattern: "trident", shotStyle: "ring", skill: "召唤仪式随从，从存活随从位置发射弹幕" },
    architect: { name: "架构师", icon: "⌬", hp: 1800, speed: 55, dmg: 38, attacks: { projectile: [13, 14] }, mechanics: { ringInterval: 2.0, ringShots: 10, ringSpeed: 160, turretInterval: 9, offsetInterval: 4.0, offsetShots: 6, offsetSpeed: 150 }, r: 44, color: "#5ad1ff", xp: 120, tier: 3, shape: "square", pattern: "nodes", shotStyle: "rune", skill: "环形弹幕 + 召唤炮台(炮台亦会射击)" },
    ...
    furnace: { name: "熔炉核心", icon: "✸", hp: 2450, speed: 30, dmg: 32, attacks: { projectile: [13] }, mechanics: { ringInterval: 3.8, ringShots: 12, ringSpeed: 180, hazardInterval: 5.2, hazardCount: 2, hazardOffset: 90, hazardRadius: 68, hazardDuration: 3.5, hazardWarm: 1 }, r: 46, color: "#ff8b46", xp: 130, tier: 3, shape: "hex", pattern: "reactor", shotStyle: "ring", skill: "双区预警灼烧 + 火环弹幕" },
