...
(function () {
  ...
  const WEAPONS = {
    ...
    boomerang: {
      ...
      tags: ["ranged"],
      stats: function (lv) {
        // 全级小幅削弱(伤害/冷却);射程=speed*life/2 随级成长:L1 154px(大幅 -45%),L8≈278px 不变
        return { damage: 12 + (lv - 1) * 3.4, count: 1 + (lv >= 3 ? 1 : 0) + (lv >= 6 ? 1 : 0), cooldown: Math.max(0.7, 1.7 - (lv - 1) * 0.085), speed: 280 + (lv - 1) * 10, life: 1.1 + (lv - 1) * 0.07, spin: 14 };
      }
    },
    grenade: {
