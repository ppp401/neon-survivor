...
(function () {
  ...
  const CONST = {
    ...
    CHAR_SKILL_LEVEL_SOFTCAP: 50, // 角色直伤技能等级软上限；之后按 sqrt(softcap*level) 成长
    // 经验
  ...
  const WEAPON_EVOS = {
    ...
    missile_railgun: Object.assign({}, WEAPONS.missile, { name: "制导天矛", color: "#ff7b68", icon: "➠", evo: true, kind: "fusion", tags: ["ranged"], fuse: ["missile_evo", "railgun_evo"], stats: function () { return { damage: 105, cooldown: 1.65, count: 2, speed: 700, seek: 6, life: 2.2, calibrate: 0.35, pierce: 99 }; } }),
  ...
  const CHARACTERS = {
    ...
    collector: { name: "磁芯", title: "拾取共鸣", icon: "✜", color: "#ffd86b", startWeapons: ["aura", "missile", "chain", "vortex", "sentry", "railgun"], startPassives: { magnet: 1 }, hpMul: 1.0, speedMul: 0.95, charMods: { pickupMul: 1.15 }, special: "collector", mechanics: { xpPerCrystal: 13, maxCrystals: 6, damageBase: 10, damagePerLevel: 0.9, radius: 28 }, ability: { trigger: "每获得 13 点实际经验蓄 1 枚，满6枚齐射", base: "每枚伤害 10+0.9×有效等级，爆裂半径28；50级后伤害成长放缓", links: "攻击、暴击、吸血、范围；磁吸经验加速充能", damageName: "磁晶齐射" }, desc: "拾取 ×1.15、移速 ×0.95；经验驱动的六枚环绕磁晶。", appearance: { shape: "circle", deco: "magnet" } },
